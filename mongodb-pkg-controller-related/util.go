...
/*
secretsUsedByPeers
For each unused Secrets :
	get the secret first
	remove it from unusedSecrets slice, if this is not found or not owned by KubeDB
EnsureOwnerReferenceForItems() called for secrets
 */
...
/*
secretsUsedByPeers first list all the Mongo object form the meta.NameSpace
Then runs loop on that mongoList and collects all the AuthSecrets and KeyFileSecrets, execpt for the currentDB(which has called this func)
 */
