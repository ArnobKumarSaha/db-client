...
// initWatcher() initializes informers, lister, queue.
// It also adds EventHandlers in the informer
...
/*
It first fetches the mongoDb object from mongoInformer. If  the object exists , check if the DeletionTimeStamp is nil or not nil
If not nil :
	So , This object is requested for gracefulDeletion by user.
	if there is no finalizer, we have nothing to do
	if there is finalizer, c.terminate(db) & util.PatchMongoDB() (with RemoveFinalizer) will be called.
if nil :
	util.PatchMongoDB() (with AddFinalizer) will be called.
	Get mongodb phase from condition.
	If new phase is not equal to old phase, update MongoDB phase.
	if conditions are empty, set initial condition "ProvisioningStarted" to "true"

	if DB object is paused , operator has nothing to do.
	if halted==true, c.halt(db) will be called.
	else Check if halted condition exists in mongodbStatus,
		if exists, remove the halted condition
		if not, c.create(db) will be called.
*/
...
// initSecretWatcher() adds eventHandlers with the secretInformer
...
// stsWatcher() adds eventHandlers with the mgStsInformer
