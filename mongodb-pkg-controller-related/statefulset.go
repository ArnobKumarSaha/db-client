...
/*
Reconcile calls ensureTopologyCluster or ensureNonTopology , based in shardTopology exists or not.
 */
...
/*
ensureTopologyCluster will be called if shardTopology is enabled.
ensureConfigNode
ensureShardNode
If statefulSet not ready , return. if verb changed , record the Event.
ensureMongosNode
set the Verb correctly and return.
 */
...
/*
ensureShardNode calls local function shardSts() & sets the Verb for each shard. nodeNum = 0 -> shard-1

<< shardSts >>
shardSts is almost same as ensureConfigNode. This works for configTopology.shard, That works for configTopology.ConfigServer
The extra things it does are (before build the workloadOptions)::
	assign storageEngine arg
	calls parseAffinityTemplate with deepCopy of podTemplate
 */
...
/*
Get MongodbVersion. Assign default values of sslMode, clusterAuth, args
installInitContainer
assign default values of cmds, envs, volumes, volumeMounts
(initial scripts)
build workloadOptions
ensureStatefulSet
 */
...
func parseAffinityTemplate(podTemplate *ofst.PodTemplateSpec, nodeNum int32) (*ofst.PodTemplateSpec, error) {
	...
	// encode the podTemplate, then evaluate it (Need to look again)
...
// *************************************************************** If shard not enabled ******************************************************************************

/*
ensureNonTopology will be called if shardTopology is not enabled.
Set them on by one ::
mongoDBVersion , sslMode, envList, clusterAuth, args
installInitContainer()
initContainers, volumes,
if there is init scripts , update volumes and volumeMounts accordingly.

If spec.ReplicaSet is not nil ::
	cmds, args, envlist, peerFinderLocation, replicasetScriptName,
	podTemplate.spec.LifeCycle, volumeMounts, volumes
ensureStatefulSet() will be called then after setting up all the workLoadOptions.
if verb is other than UnChanged :
	check if statefulSet is ready or not.
 */
...
checkStatefulSet
get MongoDbVersion
create statefulSet's ObjectMeta
assign readinessProbe & livenessProbe to nil, if all fields are zero value.
CreateOrPatchStatefulSet()
	* Set the yaml file for stateful set
CreateStatefulSetPodDisruptionBudget()
 */
...
/*
checks if statefulSet exists with a particular name
set sts's labels .. fqn, name
 */
...
Make the yaml of containers , volume & volumeMounts based in checking TLS, keyFileSecrets etc.

This function receives different types pf PodTemplate like ..
spec.PodTemplate, shardTopology.shard.PodTemplate, shardTopology.configServer.PodTemplate, shardTopology.mongos.PodTemplate
 */
...
/*
This will be called in CreateOrPatchStatefulSet inside ensureStatefulSet
For each container named with 'mongodb':
	Get the VolumeMounts & volume ; And set it into statefulSet spec
	if StorageTypeEphemeral :
		set emptyDir's sizeLimit if pvc StorageRequest exists
		assign the declared emptyDir to statefulSet's spec
	if StorageTypeDurable :
		Set the pvcSpec accessMode to readWriteOnce if not assigned yet
		make a PVC, set annotations and upsert it to statefulSet spec.
 */
...
/*
Make the MONGO_DB ROOT_USERNAME , ROOT_PASSWORD & POD_NAME env
assign them into corresponding containers
 */
...
/*
If monitoringAgent is set to prometheus, this will be called.
It will return a Container which has required-things set for Prometheus.
 */
